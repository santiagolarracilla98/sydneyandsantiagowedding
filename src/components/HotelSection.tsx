...
export function HotelSection({
  ...
  return <div className="mb-16">
      ...
      {data.categories.map((category, categoryIndex) => <div key={categoryIndex} className="mb-10">
          <h4 className="font-sans font-medium text-foreground mb-4 text-sm uppercase tracking-wider">
            {category.name}
          </h4>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {category.hotels.map((hotel, hotelIndex) => <HotelCard key={hotelIndex} hotel={hotel} language={language} />)}
          </div>
        </div>)}
    </div>;
}